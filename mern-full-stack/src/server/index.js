...
const dbroute = process.env.MONGODB_URL || `mongodb+srv://j_humildes:Senhorjesus1@cluster0-3trkt.mongodb.net/test`;
...
server.get('/api/trips/:id', (req, res) => {
  db.collection('trips').findOne({_id: new ObjectID(req.params.id) }, (err, result) => {
    if (err) throw err;
...
server.delete('/api/trips', (req, res) => {
  db.collection('trips').deleteOne( {_id: new ObjectID(req.body.id) }, err => {
...
server.post('/api/trips', (req, res) => {
  db.collection('trips').insertOne(req.body, (err, result) => {
    if (err) throw err;
...
server.put('/api/trips', (req, res) => {
  // get the ID of the user to be updated
  const id  = req.body._id;
  // remove the ID so as not to overwrite it when updating
  delete req.body._id;
  // find a user matching this ID and update their details
  db.collection('trips').updateOne( {_id: new ObjectID(id) }, {$set: req.body}, (err, result) => {
    if (err) throw err;
